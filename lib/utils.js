...
/**
 * Recipes this plugin should handle.
 *
...
/*
 * Helper to get host CLI tokens from ~/.platformsh (Fixed semantics)
 */
...
/*
 * Helper to prefix commands so they run through the exec wrapper
 */
exports.setPshExec = (cmds = [], fallback = 'app', needsPrefix = []) => _(cmds)
  ...
  // Prefix if needed
  .map(data => {
    if (_.includes(needsPrefix, data[0])) data[1] = `/helpers/upsun-exec.sh ${data[1]}`;
