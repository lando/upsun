...
const task = (service, closestApp) => ({
  service,
  description: 'Pull relationships and/or mounts from the remote Fixed environment',
  ...
  options: {
    ...
    'relationship': {
      ...
      interactive: {
        type: 'checkbox',
        message: 'Choose relationships to import from the remote Fixed environment',
        choices: () => {
    ...
    'mount': {
      ...
      interactive: {
        type: 'checkbox',
        message: 'Choose mounts to download from the remote Fixed environment',
        choices: () => {
 ...
 */
exports.getPlatformPull = (service, app = {}) => {
  const {meta, platformsh} = app;
  const pull = _.merge({}, task(service, platformsh.closestApp), {options: getAuthOptions(meta, platformsh.tokens)});
  if (app.id) {
    pull.options.project.default = app.id;
    pull.options.project.defaultDescription = app.id;
  }
  return pull;
};
