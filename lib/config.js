...
const getAppMount = (app, base, files) => {
  ...
  const match = _.find(files, {name: app.name});
  if (match && match.dir) return match.dir;
  return base;
};
...
exports.loadConfigFiles = baseDir => {
  // Flex is a hard error. Empty `.upsun/` is not Flex.
  flavor.assertFixedOnly(baseDir);
  ...
  const platformAppYamls = _(entries)
  ...
  // If we also have an applications file then concat (supports applications.yaml-only multi-app)
  if (fs.existsSync(applicationsFile)) {
    const loaded = yamlPlatform.load(applicationsFile);
    applications.push(loaded);
    _.forEach(_.flatten([loaded]), app => {
      if (app && app.name && !_.find(platformAppYamls, {name: app.name})) {
        const dir = _.has(app, 'source.root') ? path.resolve(baseDir, app.source.root) : baseDir;
        platformAppYamls.push({name: app.name, file: applicationsFile, dir});
      }
    });
  }
