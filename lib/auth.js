...
// Helper to get interactive auth options for pull/push
const getInteractiveOptions = (tokens = []) => ({
  ...
  'api-token': {
    ...
    interactive: {
      name: 'auth',
      type: 'password',
      message: 'Enter an Upsun Fixed API token (PLATFORMSH_CLI_TOKEN)',
...
// Helper to get non-interactive auth options when a token is already cached
