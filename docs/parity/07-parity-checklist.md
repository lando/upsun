...
One Fixed local OPEN is **proven (local)** for `examples/mariadb-10.4` on #221 @ `836c0b001d824477d4c372cbd809caa842b4ab73`. Pull/push are **code shipped** on main @ `9bf288b` (resume/activate, Landofile `config.id`, space-form flags, unit tests) but still **not E2E-proven** without a live `PLATFORMSH_CLI_TOKEN`. Flex and the broader OPEN matrix are still **defer**. See [(6) Image spike](06-image-spike.md).
...
| `lando pull` / `lando push` | **code shipped** | On main @ `9bf288b`. Fixed contract: `platform` + `PLATFORMSH_CLI_TOKEN`. Space-form `-r`/`-m` fixed. Landofile `config.id` → `PLATFORM_PROJECT` / `-p` / `project:set-remote`. Paused → `environment:resume`; inactive → `environment:activate`; parent fallback only if wake fails (or `--no-parent` / `--env`). Unit tests only. **Not E2E-proven** — needs a live token. |
